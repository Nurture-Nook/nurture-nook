...
def update_category(db: Session, category_id: int, category_patch: CategoryPatch) -> CategoryModView:
    ...
    updates = { key: value for key, value in category_patch.model_dump(exclude_unset=True, exclude_none=True).items() }
    ...
    return CategoryModView.model_validate(db_category)
